...
external JSPromise moveToJs(
    double lat, double lon, int zoom, int durationMs, String mapId);
...
external JSPromise fitBoundsJs(double swLat, double swLon, double neLat,
    double neLon, int durationMs, String mapId);
...
external JSPromise addPlacemarkJs(
...
external JSPromise updatePlacemarkPropertiesJs(
    String placemarkId, JSPlacemarkProperties newProperties);
...
external JSPromise updatePlacemarkOptionsJs(
    String placemarkId, JSPlacemarkOptions newOptions);
...
external JSPromise updatePolygonPropertiesJs(
    String polygonId, JSPolygonProperties newProperties);

/// JS interop: Updates polygon visual options
@JS('yandexMapController.updatePolygonOptions')
external JSPromise updatePolygonOptionsJs(
    String polygonId, JSPolygonOptions newOptions);
...
external JSPromise updatePolylinePropertiesJs(
    String polylineId, JSPolylineProperties newProps);

/// JS interop: Updates polyline visual options
@JS('yandexMapController.updatePolylineOptions')
external JSPromise updatePolylineOptionsJs(
    String polylineId, JSPolylineOptions newOpts);

/// JS interop: Performs geocoding request
@JS('yandexMapController.geocode')
external JSPromise<JSArray<JsGeocodeResult>> geocodeJs(
    JSAny request, JSGeocodeOptions options);

/// JS interop: Performs search suggestions request
@JS('yandexMapController.suggest')
external JSPromise<JSArray<JsSuggestResult>> suggestJs(
    JSString request, JSSuggestOptions options);
...
class YandexJsMapController {
  ...
  Future<void> fitBounds(PointEntity southWest, PointEntity northEast,
      ...
      await fitBoundsJs(southWest.lat, southWest.lon, northEast.lat,
              northEast.lon, durationMs, _mapId)
          .toDart;
  ...
  Future<void> setMapType(MapType type) async =>
      await setMapTypeJs(type.name, _mapId).toDart;
  ...
  Future<void> addPlacemark(PlacemarkEntity placemark) async {
    ...
    await addPlacemarkJs(jsPlacemark.geometry, jsPlacemark.properties,
            jsPlacemark.options, _mapId, placemark.id)
        .toDart;
  }
  ...
  Future<void> updatePlacemarkProperties(
      ...
      await updatePlacemarkPropertiesJs(placemarkId, newProperties.toJs())
          .toDart;
  ...
  Future<void> addPolygon(PolygonEntity polygon) async {
    ...
    await addPolygonJs(jsPolygon.geometry, jsPolygon.properties,
            jsPolygon.options, _mapId, polygon.id)
        .toDart;
  }
  ...
  Future<void> addPolyline(PolylineEntity polyline) async {
    ...
    await addPolylineJs(jsPoly.geometry, jsPoly.properties, jsPoly.options,
            _mapId, polyline.id)
        .toDart;
  }
  ...
  Future<List<GeocodeResult>> geocode<T>(T request,
      {GeocodeOptions options = const GeocodeOptions()}) async {
    final jsRequest = switch (request) {
      String s => s.toJS,
      PointEntity p => p.toJs(),
  ...
  Future<List<SuggestResult>> suggest(String request,
      {SuggestOptions options = const SuggestOptions()}) async {
    final rawResult = await suggestJs(request.toJS, options.toJs()).toDart;
    final items = rawResult.toDart.cast<JsSuggestResult>();

    return items
        .map(
            (js) => SuggestResult(displayName: js.displayName, value: js.value))
        .toList();
  }
}
